...
hept = read.table('heptathlon.dat',header=TRUE)
print(summary(hept))
X = hept[,1:7]
...
### Prefer the correlation matrix because time and distance measurements are
### too dissimilar.

# 3. How many principal component would you recover? Explain.

#library(rrcov)
#pcaX <- PcaClassic(X, scale=TRUE)
#robustbase dependency of rrcov can't be installed at cw

pcaX = princomp(cor(X))
summary(pcaX)
screeplot(pcaX,type='lines')
### Retain 3 (or 4) first PC's, they contain 95% (99%) of the information.

# 4. Interpret the first principal component.

loadings(pcaX)
### First PC is an ~average of all components.

# 5. Make a scatter plot of the scores. Interpret.

plot(hept$score)
### Scores seem to follow normal distribution.

# 6. Make a plot of the first principal score versus the score variable from
#    the data set. Interpret.

### Plot the first PC vs hept$score?
...
### How do I find the orth dist to PCA subspace?
